...
public class ConnectFour
{
	...
	public static void printBoard()
	{
		...
		{
			System.out.print("[");
			System.out.print("\033[4m"); //gives underline
			...
			System.out.print("\033[0m"); //resets formatting
			System.out.println("]");
		}
	}
	...
	{
		if (posX >= board.length) // checks if player selected row option
		{
			return false;
		}
	...
	public static boolean checkWin(char let)
	{
		...
		for (int i = board.length - 1; i >= 0; i--)
		{
			for (int j = 0; j < board[i].length; j++)
			{
				if (board[i][j] == let)
				{
					if (i >= 3) // makes sure no uneccessary checks are done (also prevents errors)
					{
						// straight up
